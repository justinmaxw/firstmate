...
test_non_cursor_launch_clears_inherited_foreign_markers() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u ANTIGRAVITY_AGENT" \
    "non-cursor launch must clear every inherited foreign identity marker"
  pass "non-cursor launches clear inherited Cursor and Antigravity identity markers"
}
