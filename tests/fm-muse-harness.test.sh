...
# markerless, so an inherited Cursor/Claude/Pi/Grok/Gemini/Antigravity marker
# would outrank the versioned muse-bin ancestor these detection cases launch.
# Drop the ambient markers so the asserted verdict does not depend on which
# harness launched the suite - agy included, since an agy crewmate runs this
# suite through a shell tool that carries ANTIGRAVITY_AGENT=1.
unset CLAUDECODE PI_CODING_AGENT FM_PI_HARNESS GROK_AGENT CURSOR_AGENT CURSOR_INVOKED_AS GEMINI_CLI ANTIGRAVITY_AGENT
