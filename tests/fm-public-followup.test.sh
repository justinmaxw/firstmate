...
# Fixture clock.
# The thread windows below are relative to one timestamp captured at suite
# start, so a seeded obligation is always a live thread with a real window
# ahead of it.
# A hardcoded calendar date silently becomes an expired thread once that date
# passes, which turns every window-checked command in this suite into a
# time-bomb failure.
# The pinned FMX_NOW_OVERRIDE values below derive from the same base, so the
# tests that need a fixed "now" stay deterministic.
FIXTURE_NOW=$(date -u +%s)
FIXTURE_THREAD_WINDOW=$((7 * 24 * 3600))
FIXTURE_OBLIGATION_WINDOW=$((30 * 24 * 3600))

fixture_iso() {  # <epoch-seconds>
  date -u -r "$1" +%Y-%m-%dT%H:%M:%SZ 2>/dev/null \
    || date -u -d "@$1" +%Y-%m-%dT%H:%M:%SZ
}

FIXTURE_EXPIRES_EPOCH=$((FIXTURE_NOW + FIXTURE_THREAD_WINDOW))
FIXTURE_RECEIVED_AT=$(fixture_iso $((FIXTURE_NOW - FIXTURE_THREAD_WINDOW)))
FIXTURE_EXPIRES_AT=$(fixture_iso "$FIXTURE_EXPIRES_EPOCH")
FIXTURE_OBLIGATION_EXPIRES_AT=$(fixture_iso $((FIXTURE_NOW + FIXTURE_OBLIGATION_WINDOW)))

# PF_TEST_NOW is the simulated "now" run_pf defaults FMX_NOW_OVERRIDE to; it
# must stay anchored to the same dynamic base as the fixture data above, or a
# frozen calendar value eventually reads seeded fixtures as received in the
# future once real time passes it.
PF_TEST_NOW=$FIXTURE_NOW
...
seed_commitment() {
  ...
  jq -n --arg r "$request" --arg p "$platform" \
    --arg received "$FIXTURE_RECEIVED_AT" --arg expires "$FIXTURE_EXPIRES_AT" \
...
seed_repro_commitment() {   # <home> <obligation> <request> <work-home> <work-id>
  ...
  jq -n --arg r "$request" --arg received "$FIXTURE_RECEIVED_AT" --arg expires "$FIXTURE_EXPIRES_AT" \
...
test_first_register_succeeds_with_empty_lock_list_under_bash32() {
  ...
  jq -n --arg received "$FIXTURE_RECEIVED_AT" --arg expires "$FIXTURE_EXPIRES_AT" \
    '{request_id:"req-empty-locks", platform:"discord",
