...
harness_marker() {
  ...
  # A fork spec once proposed an ANTIGRAVITY_AGENT=1 marker for agy (agy
  # 1.1.20), but re-verification against agy 1.2.0 found no AGY_* or
  # ANTIGRAVITY_* variable at all on a live TUI - AGENT=1 observed there is an
  # inherited launcher value, not an agy identity - so no such marker is
  # promoted here; agy is ancestry-only, like muse.
  return 0
}
