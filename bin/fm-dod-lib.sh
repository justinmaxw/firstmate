...
# fm_dod_block <no-mistakes|direct-PR|local-only> <task-id> [origin-contract-suffix]
...
# line that bin/fm-spawn.sh checks a ship brief against; the optional third argument
# is appended verbatim to that line (bin/fm-brief.sh passes " origin=none" for a
# --no-origin local-only scaffold on a genuinely remote-less project) and is
# otherwise empty.
...
fm_dod_block() {  # <mode> <task-id> [origin-contract-suffix]
  local mode=$1 id=$2 origin_contract=${3:-}
  case "$mode" in
    direct-PR)
      cat <<EOF
# Definition of done
