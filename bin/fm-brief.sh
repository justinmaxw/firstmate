...
# bin/fm-spawn.sh checks against its own explicit --mode before launching, suffixed
# " origin=none" for a --no-origin local-only scaffold on a remote-less project.
ORIGIN_CONTRACT=""
if [ "$NO_ORIGIN" -eq 1 ]; then
...
DOD=$(fm_dod_block "$MODE" "$ID" "$ORIGIN_CONTRACT") || exit 1
DOD="$DOD$SPEC_DOD"
...
EOF
if [ -n "$SPEC" ]; then
  echo "scaffolded: $BRIEF (ship, mode=$MODE, spec=$SPEC_ABS; replace {TASK} and {FIRSTMATE_SPEC})"
else
