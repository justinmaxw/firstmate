...
block_stop() {
  ...
  rule='━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'
  {
    ...
    elif [ "$FM_SUP_X_MODE" = true ]; then
      printf '●  X-mode relay polling needs supervision, but no live watcher holds this home lock (last beat: %s).\n' "$FM_SUP_BEACON_DESC"
    elif [ "$FM_SUP_NIGHT_RUN" = true ]; then
      printf '●  An armed overnight queue run needs supervision, but no live watcher holds this home lock (last beat: %s).\n' "$FM_SUP_BEACON_DESC"
    else
      printf '●  This home needs supervision, but no live watcher holds this home lock (last beat: %s).\n' "$FM_SUP_BEACON_DESC"
    fi
...
if [ "$terminal_status" -eq 0 ]; then
  if [ "$FM_SUP_IN_FLIGHT" -gt 0 ]; then
    ...
    NEED_DESC="an overnight queue run armed"
  else
    NEED_DESC="supervision required"
  fi
