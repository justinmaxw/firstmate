...
resolve_relaunch_profile() {
  ...
  if [ "$TARGET_HARNESS" = agy ]; then
    ...
    agy_profile=$("$SCRIPT_DIR/fm-harness.sh" resolve-agy-profile "$TARGET_MODEL" "$TARGET_EFFORT") \
      || die "relaunching $ID onto agy with model '$TARGET_MODEL' and effort '$TARGET_EFFORT' would stop the running agent for a launch that must be refused"
    TARGET_MODEL=${agy_profile% *}
    TARGET_EFFORT=${agy_profile#* }
  fi
