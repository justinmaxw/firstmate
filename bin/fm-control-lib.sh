...
# NOT repollute: after a single Escape it prints "Interrupted - What should
# Antigravity CLI do instead?" and leaves the composer genuinely empty
# (verified, agy 1.1.20), so it needs no clear key either. Prints the key or
# nothing; a harness with no verified mechanics returns nonzero, matching the
# tables above.
...
fm_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
    ...
    # agy has no adapter-owned structural source at all (bin/fm-busy-lib.sh
    # registers no pull or push source for it), so it carries the same 'none'
    # every other sourceless adapter carries.
