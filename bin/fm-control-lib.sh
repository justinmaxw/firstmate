...
# Which task kinds an adapter is verified to run. muse and agy are crewmate/
# scout adapters only: neither has a primary supervision protocol, and
...
# way and also does NOT repollute: after a single Escape it prints
# "Interrupted - What should Antigravity CLI do instead?" and leaves the
# composer genuinely empty (verified, agy 1.1.20), so it needs no clear key
