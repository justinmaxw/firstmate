...
#     reports a need - work in flight (state/*.meta), a registered
#     process-event source, an X-mode relay poll (state/x-watch.check.sh), or
#     an armed overnight queue run (state/night-run.check.sh); an idle home
#     exits 0.
...
# --- need: the shared supervision predicate (fm-supervision-lib.sh) ----------
need_supervision() {
