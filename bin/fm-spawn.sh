...
#   already-cleared brief and skips the pairing check. A ship or scout spawn also
#   refuses leftover `{TASK}` / `{FIRSTMATE_SPEC}`
...
#   CREWMATE/SCOUT only, matching muse and gemini, and a --secondmate spawn refuses it.
...
#   is detected, spawn skips that remote freshness check and resets to the
#   project's own local default branch instead, since the pooled worktree already
#   shares that project's object store and refs; that local fallback is allowed
#   only for a scout or a local-only ship, because a no-mistakes or direct-PR ship
#   pushes a branch and opens a PR, so a missing origin refuses the spawn before
#   any agent launches. Relaunch reuses the recorded worktree without fetching or
#   resetting its base. An unreachable detected origin, unresolved default branch,
#   or non-clean worktree refuses a fresh spawn rather than risking a PR (or, for
#   a local-only project, a ready branch) based on stale history or discarding
#   local work.
...
launch_template() {
  ...
  case "$harness" in
    ...
    # AGY_CLI_DISABLE_AUTO_UPDATE=true avoids lock contention on agy's own
    # background self-updater when several crewmates launch close together
    # (verified, agy 1.1.20; carried forward from the fork's captain-approved
    # Gemini pool spec). GEMINI_API_KEY is stripped at the launch boundary
    # itself, not only refused in fm-spawn's own environment check
    # (agy_settings_permits_subscription_only below): the pane is created by a
    # long-lived tmux/herdr daemon whose shell can export the key from an rc
    # file fm-spawn never read, and a key agy can see switches the run onto
    # pay-as-you-go API billing instead of the Google AI Pro/Ultra
    # subscription this pool exists to use.
    agy) printf '%s' 'AGY_CLI_DISABLE_AUTO_UPDATE=true env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u FM_PI_HARNESS -u GEMINI_API_KEY __AGYBIN__ --prompt-interactive "$(__OPINPUT__ encode launch-brief < __BRIEF__)" __MODELFLAG____EFFORTFLAG__--dangerously-skip-permissions' ;;
    ...
    cursor) printf '%s' 'env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u FM_PI_HARNESS -u GEMINI_CLI -u ANTIGRAVITY_AGENT -u CURSOR_INVOKED_AS __CURSORBIN__ --trust --yolo __MODELFLAG__--workspace __WORKTREE__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"' ;;
...
if [ "$KIND" = ship ]; then
  ...
  if [ "$PROJ_HAS_ORIGIN" = 0 ] && [ "$BRIEF_EXPECTS_ORIGIN" = 1 ]; then
    ...
    if [ "$MODE" = local-only ]; then
      echo "error: origin mismatch for $ID: $PROJ_NAME has no origin remote, but the brief's branch step probes one and would block the worker immediately; remove $SOURCE_BRIEF and re-scaffold it with fm-brief.sh --no-origin (the scaffold refuses to overwrite an existing brief), or start a fresh task" >&2
    else
  ...
  if [ "$PROJ_HAS_ORIGIN" = 1 ] && [ "$BRIEF_EXPECTS_ORIGIN" = 0 ]; then
    echo "error: origin mismatch for $ID: the brief was scaffolded with --no-origin, but $PROJ_NAME does have an origin remote; remove $SOURCE_BRIEF and re-scaffold it without --no-origin (the scaffold refuses to overwrite an existing brief), or start a fresh task, so its branch step matches the project" >&2
    exit 1
  fi
...
freshen_spawn_worktree_base() {  # <worktree> <allow-no-origin>
  ...
  # and its local default branch ref is already the freshest truth available,
  # shared with every worktree of that repo with no fetch delay.
  #
