...
| `fm-tidy.sh`             | Remove a home's expired dead operational records, standalone (`--dry-run`) or as bootstrap's locked tidy sweep |
...
| `fm-brief.sh`            | Scaffold ship (explicit `--mode`, optionally remote-less `--no-origin`), scout, secondmate-charter, Herdr-lab, and spec-backed (`--spec`) briefs, with Captain's intent and Firstmate spec subsections on ship/scout |
...
| `fm-secondmate-compact.sh` | Stow, then compact, one idle secondmate's conversation context, refusing anything it could not confirm |
