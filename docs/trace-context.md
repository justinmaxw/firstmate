...
The injection sits at the unconditional pre-launch export site, so it covers ship and scout spawns on every verified harness adapter (`AGENTS.md` section 4 owns that roster), plus Secondmate spawns on that same set minus the crewmate/scout-only adapters `bin/fm-spawn.sh` refuses for a Secondmate.
