...
agy (Antigravity CLI) is likewise not covered by this matrix and does not reach a proven `empty`: its idle composer's bare-`─`-rule box collides with the scanner's existing Pi-pair detector, so a genuinely idle agy pane classifies `unknown` rather than `empty` (safe, never a wrong verdict) - see the [Antigravity CLI (agy)](#antigravity-cli-agy) section below for the live evidence and root cause.
...
Evidence produced 2026-08-26 on macOS 25.5.0 arm64, real Antigravity CLI 1.1.20, the captain's own authenticated Google AI Pro session, `GEMINI_API_KEY` confirmed unset throughout.

- **tmux - VERIFIED**, the reference backend, cleared first per this criterion's own requirement. A real isolated tmux server (`tmux -L <private-socket>`) launched `agy -i '<prompt>' --model gemini-3.7-flash --effort low --dangerously-skip-permissions` in a genuinely never-trusted worktree. Observed and confirmed live: the workspace-trust dialog, a real subscription-backed reply naming the exact requested word, the verified busy footer (`esc to cancel`) matching `fm_busy_lines_match agy` throughout the turn and clearing on completion, a clean single-Escape interrupt producing `⎿ Interrupted · What should Antigravity CLI do instead?` with the live composer prompt row left as a bare `>` (no restored text, confirming no clear key is needed), and `/exit` returning cleanly with a working `--conversation=<uuid>` resume hint. `tests/fm-agy-signals-live-e2e.test.sh` (`FM_AGY_SIGNALS_LIVE=1`) is the refresh command and pins all of the above except the exit/resume check, which was confirmed manually. One disclosed, safe limitation was found and is NOT part of this VERIFIED claim: the shared structural composer classifier (`fm_tmux_composer_state`) reads agy's genuinely idle composer as `unknown` rather than `empty` - see [Composer classification matrix](#composer-classification-matrix) and the harness-adapters skill's agy section "Known limitation" for the root cause and why it is out of proportion to fix within this adapter's own task.
- **herdr - VERIFIED for launch, trust, and reply; not exercised beyond that.** An isolated lab session (`bin/fm-herdr-lab.sh`, never the captain's live `default` session) created a workspace and tab, then `herdr pane run` typed and submitted the same launch command atomically. The workspace-trust dialog appeared identically to tmux and was accepted the same way; the real subscription-backed reply then confirmed the exact requested word. Interrupt, exit/resume, and composer-classification behavior were NOT independently re-verified on herdr in this pass - `bin/backends/herdr.sh` needed no agy-specific code change at all (its `agent get`/`agent_status` liveness signal and submit-confirmation path are harness-name-agnostic, unlike tmux's `fm_backend_tmux_classify_process_name`), so there is no herdr-specific logic this adapter introduces that tmux's evidence does not already cover; a live herdr guard mirroring `tests/fm-agy-signals-live-e2e.test.sh` is real follow-up work, not part of this claim.
- **zellij - NOT YET VERIFIED.** No agy-specific code exists in any zellij backend file (none is in this task's scope), and no live session was attempted. Zellij is never automatically selected as a runtime backend (`docs/configuration.md` "Runtime backend"), so an operator must explicitly opt in before this gap is reachable.
- **orca - NOT YET VERIFIED.** Same reasoning as zellij: no agy-specific code exists, no live session was attempted, and Orca is an experimental backend selectable only explicitly or by auto-detection under its own separate contract (`docs/orca-backend.md`).
- **cmux - NOT YET VERIFIED.** Same reasoning again: no agy-specific code exists and no live session was attempted; cmux is an experimental backend.

None of the three not-yet-verified backends were silently skipped: each was considered and is named here explicitly, per this criterion's own requirement, rather than omitted.

### Composer classification limitation, live evidence

Live-verified (Antigravity CLI 1.1.20, 2026-08-26, real isolated tmux server): `fm_tmux_composer_state` against a genuinely idle agy pane returns `unknown`, and direct calls to `fm_composer_classify_screen` against the real captured screen return `need-identity` in BOTH cursor-anchored and cursorless modes (ruling out Cursor's own `fm_tmux_pane_is_cursor` retry pattern as a transferable fix, since that pattern only helps when the ambiguity is about cursor placement, not screen content).
Root cause: agy's composer box is bounded by a bare `─` rule both above and below the prompt row, with no corner glyphs, which is structurally identical to what `bin/fm-composer-lib.sh`'s `_fm_composer_pi_separator_row` exists to recognize as Pi's own composer.
Resolving a detected pi-pair candidate requires proving Pi process identity (`fm_tmux_composer_identity`), which agy correctly fails, so the verdict safely falls through to `unknown` rather than guessing `empty` or `pending`.
`tests/fm-agy-signals-live-e2e.test.sh` pins this exact live behavior rather than asserting the more precise `empty` a future fix to the shared scanner could earn.
