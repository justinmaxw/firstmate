...
agy (Antigravity CLI, the captain's Google AI Pro/Ultra subscription pool) is likewise verified for crewmate and scout launches ONLY, refused for a secondmate for the same reason as muse - no confirmed hook/lifecycle surface exists for it to build a primary supervision protocol on.
`fm-spawn.sh` refuses an agy launch unless `~/.gemini/antigravity-cli/jetski_state.pbtxt` is present and non-empty, unless the resolved model is the literal `gemini-3.7-flash`, and unless none of `GEMINI_API_KEY`, a settings.json `modelProvider`, or `useG1Credits: true` is set to route the run onto API billing or personal-credit top-ups instead of the subscription; [`.agents/skills/harness-adapters/SKILL.md`](../.agents/skills/harness-adapters/SKILL.md#agy-verified-crewmatescout-2026-08-26-antigravity-cli-1120) owns the complete evidence, including a disclosed limitation in shared composer-emptiness detection.
Dispatch-profile wiring for agy is separate later work, so a `config/crew-dispatch.json` profile naming it is still reported as an unverified harness; until that lands, select agy with an explicit `--harness agy` or a `config/crew-harness` pin.
...
FM_BOOTSTRAP_LOCKED=0   # internal session-start signal that this session holds the fleet lock: gates bootstrap's tidy sweep and selects locked repair wording in detect-only mode; see bin/fm-bootstrap.sh
