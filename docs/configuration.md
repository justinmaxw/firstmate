...
`fm-spawn.sh` refuses an agy launch unless `~/.gemini/antigravity-cli/jetski_state.pbtxt` is present and non-empty, unless the resolved model is the literal `gemini-3.7-flash`, and unless none of `GEMINI_API_KEY`, a settings.json `modelProvider`, or `useG1Credits: true` is set to route the run onto API billing or personal-credit top-ups instead of the subscription; [its adapter reference](../.agents/skills/harness-adapters/references/harness/agy.md) owns the complete evidence, including a disclosed limitation in shared composer-emptiness detection.
