...
The same placement-specific launch also propagates the primary's declared inherited local material: `config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, `config/backend`, `config/herdr-presentation-spaces`, `config/startup-memory-budget`, `config/trace-context`, and the one shared captain-preference file `data/captain-shared.md`.
The locked bootstrap sweep re-converges that set except for session-scoped `config/trace-context`, whose current-home decision stays frozen until the next launch.
