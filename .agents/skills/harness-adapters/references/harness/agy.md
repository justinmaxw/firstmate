...
| Effort | Encoded in the model id's suffix, so no separate `--effort` flag is passed (verified live, agy 1.2.2: `--model gemini-3.7-flash-medium` alone is accepted). This replaces agy 1.1.20, whose bare `gemini-3.7-flash` required a separate `--effort` flag. `../../../../../bin/fm-harness.sh resolve-agy-profile` owns one rule for both spawn and relaunch: model and effort both absent resolves to `gemini-3.7-flash-medium`; an effort with no model resolves to the matching variant (`low` gives `gemini-3.7-flash-low`); a model with no effort launches that model; any model outside `gemini-3.7-flash-low\|medium\|high` refuses; and a model and effort that disagree refuse rather than silently picking one. `state/<id>.meta` always records the resolved model's suffix as the effort (`gemini-3.7-flash-medium` records `effort=medium`). `../../../../../bin/fm-control.sh relaunch` drops a recorded effort the caller did not restate (and a recorded model when only `--effort` is given), then applies that same rule before anything is stopped, so a refused relaunch - including a task recorded on a pre-catalog model such as bare `gemini-3.7-flash` - leaves the running agent untouched. |
