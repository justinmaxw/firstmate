...
`/compact`, typed into the composer, compacts a persistent secondmate's growing conversation through `../../../../../bin/fm-secondmate-compact.sh`, which owns the whole sequence and its refusals; the same slash-popup submit hazard as `$`/`/` elsewhere applies, and the shared submit retry lands it.
Confirmation is a transcript entry carrying `"isCompactSummary":true` with the compaction's own `"timestamp"`, appended under `${CLAUDE_CONFIG_DIR:-$HOME/.claude}/projects/<slug>/`, where `<slug>` is the session's cwd with every non-alphanumeric character replaced by a dash.
On a short conversation Claude answers `Not enough messages to compact.` and nothing is compacted, reported as an unconfirmed compaction.
Claude Code turns transcript saving OFF when it inherits a `CLAUDE_CODE_CHILD_SESSION` marker, so a secondmate launched from a nested Claude session writes no signal at all; `CLAUDE_CODE_FORCE_SESSION_PERSISTENCE=1` restores it.
...
Claude is the only harness with a live-verified compaction command AND completion signal today; `../../../../../bin/fm-secondmate-compact.sh` refuses every other harness rather than guessing.
