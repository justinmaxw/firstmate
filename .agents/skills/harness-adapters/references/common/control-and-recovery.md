...
A confirmed submit means the target accepted the text, and a positively confirmed swallowed Enter is a distinct non-zero result meaning the steer did not land; an unconfirmed or unknown verdict against a busy worker is a third case where the steer is queued and will deliver, not lost, so inspect the target's steering queue before re-sending rather than re-sending on that error alone.
