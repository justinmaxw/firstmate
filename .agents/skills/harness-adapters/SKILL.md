...
| agy | `--model gemini-3.7-flash` (the only allowed value) | `--effort <low\|medium\|high>` | Verified 2026-08-26 on Antigravity CLI 1.1.20. Unlike every other adapter, `--effort` is REQUIRED whenever `--model gemini-3.7-flash` is given - omitting it fails loudly (`--model gemini-3.7-flash requires --effort`) - so `bin/fm-spawn.sh` always emits one, defaulting to `medium` when none is requested and folding `xhigh`/`max` onto `medium` rather than omitting the flag. |
...
| agy | Run `agy models`, which lists the current account's catalog. Firstmate never selects from it: the ONLY allowed model is the literal `gemini-3.7-flash`, enforced in `bin/fm-spawn.sh` itself regardless of what the catalog otherwise lists. |
...
- agy: no verified skill invocation. Its `/` popup discovers only its own built-in slash commands, not firstmate's user-level skills (verified: `/no-mistakes` returned "No matches"); use natural language, the same fallback opencode and pi use.
...
These three cases are the TYPED plane, where `bin/fm-send.sh` types the line once, retries Enter only, and exits non-zero for an inconclusive send without claiming that the text was swallowed.
An ordinary text steer to a recorded task rides the durable inbox instead, where the record is the delivery and no pane verdict proves or disproves it; `bin/fm-send.sh`'s header owns both planes.
...
## agy (VERIFIED CREWMATE/SCOUT 2026-08-26, Antigravity CLI 1.1.20)

Antigravity CLI (binary `agy`) is the captain's Google AI Pro subscription pool, built from the captain-approved Gemini Pro pool spec (`data/firstmate-gemini-pro-pool-spec-260825/report.md` in the primary home).
It is a CREWMATE and SCOUT adapter only, the same scope and reasoning as muse: no confirmed hook/lifecycle surface exists for it to build a primary supervision protocol on, and `bin/fm-spawn.sh` refuses a `--secondmate` launch on it.
This section covers only Task 1 - the standalone adapter. `lastResort` dispatch/quota wiring is separate, later work; today an `agy` worker is launched only by an explicit `--harness agy` spawn or a static `config/crew-harness` pin, and a `config/crew-dispatch.json` profile naming it is still reported as an unverified harness by bootstrap validation until that wiring lands.

| Fact | Value |
|---|---|
| Binary | Executable `agy` from `PATH`, resolved to an absolute path; spawning refuses if it is absent. The live process name carries no version suffix and no wrapper exec, unlike muse's `muse-bin-<version>` (verified: `ps -o comm=` on a running interactive pane is exactly `agy`). |
| Launch | `-i`/`--prompt-interactive` starts the supervised interactive session with the brief as its own flag value - a positional-prompt-then-interactive shape, the same family as grok/muse/pi. |
| Models | The ONLY allowed model is the literal `gemini-3.7-flash`, enforced in `bin/fm-spawn.sh` itself (captain-approved constraint); an explicit request for anything else refuses the launch, and an empty/default request resolves to the pinned identity on this adapter's own launch template rather than falling through to whatever `agy` would otherwise pick. A raw launch command is refused outright for `harness=agy` (`bin/fm-spawn.sh`): it carries none of the placeholders this allowlist, the credential preflight, and the subscription-only billing refusal are gated on, so it would bypass all three at once - pass `--harness agy` or the bare `agy` positional instead. `agy models` lists no bare `gemini-3.7-flash` entry (only effort-suffixed slugs like `gemini-3.7-flash-medium`), but the flag parser accepts the bare family name directly and the interactive banner echoes back the exact pinned identity. |
| Busy state | No source is registered in `bin/fm-busy-lib.sh`: no confirmed hook/lifecycle surface exists for agy's interactive mode, and its own conversation storage is an undocumented, actively-written, per-conversation SQLite file (`~/.gemini/antigravity-cli/conversations/<uuid>.db`, `steps`/`trajectory_meta` tables with no vendor-published schema) - not a safe structural source to poll. `fm_busy_classify` reports `agy` `unknown` always, matching standalone Kimi before its own verification. The verified rendered footer (`esc to cancel` when busy, `? for shortcuts` when idle) is a DELIVERY-confirmation signal only, and it sits in both `bin/fm-composer-lib.sh`'s per-harness `agy` row and its harness-less union - the submit cores read a pane with no recorded harness, so without the union entry a delivered agy Enter would be reported as never submitted. |
| Exit command | `/exit` (also bound to `ctrl+d`); prints `Resume with -c (or command below): agy --conversation=<uuid>` and returns cleanly to the shell. |
...
| Skill invocation | None verified; its `/` popup discovers only its own built-in commands, not firstmate's user-level skills (`/no-mistakes` returned "No matches"). Use natural language, like opencode/pi. |
| Autonomy | `--dangerously-skip-permissions` (the same name and shape as Claude's own flag): auto-approves every tool call - shell, file write, browser, MCP, and subagent spawning - with no per-call prompt. No narrower unattended mode exists: the default `request-review` permission mode blocks on every tool call including trivial ones, which is unusable for an unattended crewmate. This carries the same disclosed recursive-subagent risk every other full-autonomy adapter flag already carries fleet-wide; Claude's own compensating guard (`bin/fm-subagent-pretool-check.sh`) is PRIMARY-scoped, so Claude crewmates carry this identical risk today, and agy's crewmate/scout-only scope keeps its exposure no wider than that existing baseline. |
| Trust dialog | `--dangerously-skip-permissions` does NOT suppress the separate first-launch-per-path workspace-trust dialog ("Do you trust the contents of this project?"), verified against a genuinely never-trusted directory. It persists per path like codex/pi/cursor's own dialogs; peek the pane within about 20 seconds of spawn and accept with Enter if shown, the same post-spawn handling codex and pi already need. |
| Environment marker | `ANTIGRAVITY_AGENT=1`, set for agy's child/tool processes (verified live, agy 1.1.20, from a real shell tool child's own `env` dump - does NOT set `CLAUDECODE` despite sharing Claude's `--dangerously-skip-permissions` flag name). Unambiguous when present but not guaranteed present for every future agy subprocess shape, so `bin/fm-harness.sh`'s ancestry walk on the exact command name `agy` (anchored, not globbed, same short-name collision caution as `pi`) remains the guarantee, the same fast-path-vs-guarantee split as grok's own marker. `fm-spawn.sh` clears it at the shared launch boundary that covers claude, codex, opencode, pi, pi-signed, grok, kimi, muse, and agy - the boundary that matters, since codex, opencode, and kimi are markerless and would otherwise report `agy` from an inherited marker - and muse's and cursor's own templates clear it too alongside the other foreign markers they already clear. |
| Composer | A rule-bordered box (a bare `─` rule immediately above and below the prompt row, no corner glyphs) whose prompt glyph is a bare `>`, styled `\x1b[94m` (bright blue, an indexed color, not truecolor). No idle placeholder or ghost text was observed. |
| Effort | `--effort <low\|medium\|high>`. Unlike every other harness, `agy`'s own CLI REQUIRES this flag whenever `--model gemini-3.7-flash` is given - omitting it fails loudly rather than falling back to a default - so `bin/fm-spawn.sh` always emits one: `medium` when the caller names none (captain-approved default), and `medium` again when an unsupported class (`xhigh`, `max`) is requested, preserving launch success instead of passing a value `agy` would reject. The resolved value is recorded in `state/<id>.meta` rather than the flagless `effort=default` other adapters record, because agy has no flagless mode for that to describe; the raw launch command escape hatch is refused for `agy` (see the Models row), so no reachable path records the flagless `effort=default` for it. |
...
Modeled directly on the muse precedent ("Credentials are a spawn preflight, not a screen check"): `bin/fm-spawn.sh` refuses an `agy` launch unless `~/.gemini/antigravity-cli/jetski_state.pbtxt` exists and is non-empty, because an unauthenticated interactive pane does not exit - it sits on a Google sign-in browser flow indefinitely, which supervision would misread as a wedged worker rather than a missing credential. The file's contents are never read; only its presence is checked. Verified live: every subsequent local `agy` process, including a separately spawned OS process, silently reuses the captain's one interactive login with no re-prompt.

Two further captain-approved refusals sit alongside the credential check, both fail-closed on any `jq` error (a settings file that cannot be parsed is refused, never silently permitted): a set `GEMINI_API_KEY`, or a `settings.json` `modelProvider` key, both switch `agy` onto pay-as-you-go API billing instead of the Google AI Pro/Ultra subscription this pool exists to use, and refuse the launch; a `settings.json` `useG1Credits: true` would silently spend the captain's own paid personal-credit top-ups once subscription quota is exhausted, and also refuses. The captain's own post-login `settings.json` was verified to hold only `trustedWorkspaces` - none of the three refused conditions - so a clean install passes through unaffected.

The `GEMINI_API_KEY` refusal reads fm-spawn's OWN process environment, which is not the pane's: crewmate panes are created by a long-lived tmux/herdr daemon whose shell can export the key from an rc file firstmate never read, the same caller-vs-worker environment split muse's `META_API_KEY` handling already documents. That refusal is therefore an early diagnostic, not the guarantee. The guarantee rides the launch itself - agy's generated launch carries `env -u GEMINI_API_KEY`, so the actual `agy` child cannot see the key on any backend even when the caller-side check saw nothing.

Open follow-up, deliberately not implemented: the wider Gemini CLI tooling family also honors a `GOOGLE_GENAI_USE_VERTEXAI`/`GOOGLE_CLOUD_PROJECT` environment-variable Vertex vector.
Whether `agy` itself reads those is UNVERIFIED, and this guard refuses only vectors verified first-hand against agy 1.1.20, so refusing on them today would block launches on no evidence.
Verify the vector against the vendor CLI first, then extend `agy_settings_permits_subscription_only` in `bin/fm-spawn.sh`.

### Known limitation: composer emptiness is not provable yet

`bin/fm-tmux-lib.sh`'s shared structural composer classifier (`fm_tmux_composer_state`) reads a genuinely idle `agy` pane as `unknown`, never `empty`, live-verified (Antigravity CLI 1.1.20, 2026-08-26). This is SAFE - it never misreports a real idle composer as `pending`, and never misreports one as falsely proven `empty` either - but it does mean any convenience gated on a PROVEN-empty composer (for example away-mode auto-injection) is unavailable for `agy` today.

Root cause: `agy`'s composer box uses a bare `─` rule both above and below its prompt with no corner glyphs, and `bin/fm-composer-lib.sh`'s existing `_fm_composer_pi_separator_row`/pi-pair detector matches that exact shape - it exists to recognize Pi's own composer, which is drawn the same way. Resolving a detected pi-pair candidate requires proving Pi process identity (`fm_tmux_composer_identity`), which `agy` correctly fails, so the verdict falls through to the safe `unknown` rather than a wrong guess. Cursor's own precedent (`fm_tmux_pane_is_cursor`, a post-hoc structural-identity reclassification) does not transfer directly: that fix retries with the cursor row dropped, but agy's ambiguity is a property of the SCREEN CONTENT (the bare-rule pair itself), not of cursor placement, so the identical retry still returns the same unresolved pi-pair candidate (verified: both the cursor-anchored and cursorless calls against agy's real captured screen return `need-identity`).

A real fix needs either teaching the shared scanner a non-Pi identity path that can validate a pi-pair-shaped box as a genuine composer (extending `_fm_composer_pi_verdict`'s pi-only gate), or a different structural disambiguator entirely - both are meaningful changes to a shared, heavily-verified library used by every harness on every backend, out of proportion for this adapter's own Task 1 scope, and each would need the same dual portable-plus-live-guard verification this skill's "Harness-dependent checks" section requires, re-run against every harness that already depends on the scanner's current behavior. `tests/fm-agy-signals-live-e2e.test.sh` pins the current, honest `unknown` verdict live rather than asserting the more precise `empty` a future fix could earn.

### Maturity caveats

Antigravity CLI is closed-source (`google-antigravity/antigravity-cli` carries `licenseInfo: null`) and self-updating; `AGY_CLI_DISABLE_AUTO_UPDATE=true` is set on every firstmate-launched `agy` worker to avoid lock contention on its background self-updater's 15-minute-debounced advisory lock (`~/.gemini/antigravity-cli/updater/update.lock`) when several crewmates launch close together, matching the captain-approved spec's Stage 0 addendum. There is no published numeric subscription quota in Antigravity's own public documentation (only qualitative "high"/"most generous" language per tier); a real numeric quota IS available from the authenticated product itself (`agy -p "/usage"`, verified live), but `quota-axi`'s own `agy` probe is a loopback check expecting an already-running `agy` process and finds nothing against a one-shot call - wiring that surface is separate, later work (the spec's Task 2), not part of this adapter. A live agy process sets `ANTIGRAVITY_LS_ADDRESS=localhost:<port>` for its own child/tool processes (verified live, agy 1.1.20), naming a local loopback address; this is plausibly the same loopback service `quota-axi`'s existing probe already expects, worth investigating first rather than assuming a new mechanism is needed, but that investigation is Task 2's, not this one's.
