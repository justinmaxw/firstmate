...
| Models | The ONLY allowed model is the literal `gemini-3.7-flash`, enforced in `bin/fm-spawn.sh` itself (captain-approved constraint); an explicit request for anything else refuses the launch, and an empty/default request resolves to the pinned identity on this adapter's own launch template rather than falling through to whatever `agy` would otherwise pick. A raw launch command receives no model placeholder, so it spells its own model out and task metadata keeps the flagless `model=default`, the same split the effort row below describes. `agy models` lists no bare `gemini-3.7-flash` entry (only effort-suffixed slugs like `gemini-3.7-flash-medium`), but the flag parser accepts the bare family name directly and the interactive banner echoes back the exact pinned identity. |
