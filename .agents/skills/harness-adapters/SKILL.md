...
This section covers only Task 1 - the standalone adapter. `lastResort` dispatch/quota wiring is separate, later work; today `agy` is reachable only through an explicit `--harness agy` spawn.
...
| Models | The ONLY allowed model is the literal `gemini-3.7-flash`, enforced in `bin/fm-spawn.sh` itself (captain-approved constraint); an empty/default request resolves to it rather than falling through to whatever `agy` would otherwise pick. `agy models` lists no bare `gemini-3.7-flash` entry (only effort-suffixed slugs like `gemini-3.7-flash-medium`), but the flag parser accepts the bare family name directly and the interactive banner echoes back the exact pinned identity. |
